    ...
    const fetchStats = async () => {
      try {
        const [statsRes, weightRes] = await Promise.all([
          axios.get(
            `http://localhost:5000/api/attendance/stats?month=${month}&year=${year}`,
            { headers: { Authorization: token } },
          ),
          axios.get(
            `http://localhost:5000/api/attendance/weightage?month=${month}&year=${year}`,
            { headers: { Authorization: token } },
          ),
        ]);
      ...
      <div style={styles.wrapper}>
        ...
        <div style={styles.headerBtns}>
          <Link to="/my-attendance">
            <button style={styles.myAttBtn}>👤 My Attendance</button>
          </Link>
          <Link to="/dashboard">
                ...
                {employees.map((emp, i) => {
                  ...
                  const hours =
                    att.check_in && att.check_out
                      ? (
                          (parseInt(att.check_out) * 60 +
                            parseInt(att.check_out?.split(":")[1]) -
                            (parseInt(att.check_in) * 60 +
                              parseInt(att.check_in?.split(":")[1]))) /
                          60
                        ).toFixed(1)
                      : att.status === "present"
                        ? "8.0"
                    ...
                    <tr key={emp.employee_id} style={styles.tr}>
                      ...
                      <td style={styles.td}>
                        <select
                          style={{
                            ...styles.statusSelect,
                            backgroundColor: STATUS_COLORS[att.status]?.bg,
                            color: STATUS_COLORS[att.status]?.color,
                          }}
                          value={att.status}
                          onChange={(e) =>
                            handleStatusChange(
          ...
          {stats && (
            <>
              ...
              <div style={styles.statsGrid}>
                {[
                  ...
                  <div
                    ...
                    style={{
                      ...styles.statCard,
                      borderTop: `4px solid ${s.bg}`,
                    }}
                  >
                    <div
                      style={{ fontSize: 28, fontWeight: "700", color: s.bg }}
                    >
                      {s.value}
                    </div>
                      ...
                      <PieChart>
                        <Pie
                          data={[
                            {
                              name: "Present",
                              value: parseInt(stats.today.present) || 0,
                            },
                            {
                              name: "Absent",
                              value: parseInt(stats.today.absent) || 0,
                            },
                            {
                              name: "Half Day",
                              value: parseInt(stats.today.half_day) || 0,
                            },
                            {
                              name: "Late",
                              value: parseInt(stats.today.late) || 0,
                            },
                          ].filter((d) => d.value > 0)}
                          cx="50%"
                          cy="50%"
              ...
              <div style={styles.chartCardFull}>
                <h3 style={styles.chartTitle}>
                  ⚖️ Employee Attendance Weightage (%) — {month}/{year}
                </h3>
                <p style={styles.chartSub}>
                  Present=100% • Late=75% • Half Day=50% • Absent=0%
                </p>
                {weightage.length === 0 ? (
                  <div style={styles.noData}>
                    No weightage data for this month
                  </div>
                ) : (
                    ...
                    >
                      ...
                      >
                        {weightage.map((entry, i) => (
                          <Cell
                            key={i}
                            fill={getWeightageColor(entry.weightage)}
                          />
                        ))}
                      </Bar>
                    </BarChart>
                          ...
                          <td style={styles.td}>
                            <div style={styles.empAvatar}>
                              <div
                                style={{
                                  ...styles.avatarCircle,
                                  backgroundColor: getWeightageColor(
                                    w.weightage,
                                  ),
                                }}
                              >
                          ...
                          <td style={styles.td}>
                            <div style={styles.weightageCell}>
                              ...
                              <span
                                style={{
                                  ...styles.weightagePct,
                                  color: getWeightageColor(w.weightage),
                                }}
                              >
                                {w.weightage || 0}%
                              </span>
...
const styles = {
  wrapper: { fontFamily: "'Inter','Segoe UI',sans-serif",
