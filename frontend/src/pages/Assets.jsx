  ...
  const fetchAll = useCallback(async () => {
    try {
      ...
      const [assetsRes, allocatedRes, statsRes, usersRes] = await Promise.all([
        axios.get(
          `http://localhost:5000/api/assets?search=${search}&status=${statusFilter}`,
          { headers },
        ),
    ...
    <Layout>
      <div style={styles.wrapper}>
        ...
        <button
          style={{
            ...styles.tab,
            ...(tab === "allocated" ? styles.activeTab : {}),
          }}
        ...
        <>
          ...
          {showForm && (
            <form onSubmit={handleCreateAsset} style={styles.form}>
              <input
                style={styles.input}
                placeholder="Asset Code (e.g. AST009)"
              ...
              <input
                ...
                onChange={(e) =>
                  setNewAsset({ ...newAsset, purchase_cost: e.target.value })
                }
                required
              />
              ...
              <tbody>
                {assets.map((a) => (
                  <tr key={a.id} style={styles.tr}>
                    ...
                    <td style={styles.td}>
                      ₹{a.purchase_cost?.toLocaleString()}
                    </td>
                    <td style={styles.td}>
                      <span
                        style={{ ...styles.badge, ...getStatusStyle(a.status) }}
                      >
        ...
        <div style={styles.allocateCard}>
          ...
          <form onSubmit={handleAllocate} style={styles.form}>
            <select
              ...
              onChange={(e) =>
                setAllocateForm({ ...allocateForm, asset_id: e.target.value })
              }
              required
            >
