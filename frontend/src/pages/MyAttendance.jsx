    ...
    const fetch = async () => {
      try {
        ...
        const weightage =
          total > 0
            ? (
                ((present * 1.0 + late * 0.75 + halfDay * 0.5) / total) *
                100
              ).toFixed(1)
            : 0;
        ...
        <>
          ...
          <div style={styles.statsGrid}>
            {[
              ...
              >
                <div style={{ fontSize: 28, fontWeight: "700", color: s.bg }}>
                  {s.value}
                </div>
          ...
          <div style={styles.weightageCard}>
            <div style={styles.weightageLeft}>
              <div style={styles.weightageTitle}>
                ⚖️ My Attendance Weightage
              </div>
              <div style={styles.weightageDesc}>
                Based on last {summary.total} recorded days
              </div>
              <div style={styles.weightageFormula}>
            ...
            <div style={styles.weightageRight}>
              <div
                style={{
                  fontSize: 52,
                  fontWeight: "800",
                  color: summary.weightage >= 75 ? "#059669" : "#dc2626",
                }}
              >
                {summary.weightage}%
              </div>
              <div style={styles.weightageStatus}>
                {summary.weightage >= 90
                  ? "🌟 Excellent"
                  : summary.weightage >= 75
                    ? "✅ Good"
                    : summary.weightage >= 50
                      ? "⚠️ Average"
                      : "❌ Poor"}
              </div>
            </div>
          ...
          {chartData.length > 0 && (
            <div style={styles.chartCard}>
              <h3 style={styles.chartTitle}>
                📊 Daily Working Hours (Last 20 Days)
              </h3>
              ...
              {records.length === 0 ? (
                ...
                records.map((r, i) => (
                  <tr key={r.id} style={styles.tr}>
                    ...
                    <td style={styles.td}>
                      <span
                        style={{ ...styles.badge, ...getStatusStyle(r.status) }}
                      >
