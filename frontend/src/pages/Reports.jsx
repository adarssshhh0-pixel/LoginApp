...
const COLORS = ["#1d4ed8","#059669","#d97706","#dc2626","#0891b2","#7c3aed","#be123c","#0f766e"];
    ...
    <div style={styles.wrapper}>
      <div style={styles.header}>
        <div>
          ...
          <p style={styles.pageSub}>Export and analyze your organization data</p>
        </div>
        ...
        <>
          ...
          <div style={styles.tableWrapper}>
            <table style={styles.table}>
              ...
              <tbody>
                {employees.length === 0 ? (
                  <tr><td colSpan={7} style={styles.empty}>No employees found</td></tr>
                ) : employees.map((e, i) => (
                  <tr key={i} style={styles.tr}>
                    <td style={styles.td}>{i + 1}</td>
                    <td style={styles.td}><span style={styles.empName}>{e.name}</span></td>
                    <td style={styles.td}>{e.email}</td>
                    <td style={styles.td}><span style={styles.deptBadge}>{e.department_name}</span></td>
      ...
      {tab === "leaves" && !loading && (
        <>
          <div style={styles.exportBar}>
            ...
            <button style={styles.exportBtn} onClick={() => exportToExcel(leaves, "Leave_Report")}>
              📥 Export Excel
            </button>
          </div>
          <div style={styles.tableWrapper}>
            <table style={styles.table}>
              ...
              <tbody>
                {leaves.length === 0 ? (
                  <tr><td colSpan={7} style={styles.empty}>No leave records found</td></tr>
                ) : leaves.map((l, i) => (
                  <tr key={i} style={styles.tr}>
                    <td style={styles.td}>{i + 1}</td>
                    <td style={styles.td}><span style={styles.empName}>{l.employee_name}</span></td>
      ...
      {tab === "assets" && !loading && (
        <>
          <div style={styles.exportBar}>
            ...
            <button style={styles.exportBtn} onClick={() => exportToExcel(assets, "Asset_Report")}>
              📥 Export Excel
            </button>
          </div>
          <div style={styles.tableWrapper}>
            <table style={styles.table}>
              ...
              <tbody>
                {assets.length === 0 ? (
                  <tr><td colSpan={7} style={styles.empty}>No assets found</td></tr>
                ) : assets.map((a, i) => (
                  <tr key={i} style={styles.tr}>
                    <td style={styles.td}>{i + 1}</td>
                    ...
                    <td style={styles.td}>₹{Number(a.purchase_cost).toLocaleString()}</td>
                    <td style={styles.td}>
                      <span style={{ ...styles.badge, backgroundColor: a.status === "available" ? "#dcfce7" : "#dbeafe", color: a.status === "available" ? "#059669" : "#1d4ed8" }}>
      ...
      {tab === "attendance" && !loading && (
        <>
          <div style={styles.exportBar}>
            <select style={styles.filterSelect} value={attMonth} onChange={(e) => setAttMonth(e.target.value)}>
              {["January","February","March","April","May","June","July","August","September","October","November","December"].map((m,i) => (
                <option key={i} value={i+1}>{m}</option>
              ))}
            </select>
            <select style={styles.filterSelect} value={attYear} onChange={(e) => setAttYear(e.target.value)}>
              {[2024,2025,2026].map(y => <option key={y} value={y}>{y}</option>)}
            </select>
            <span style={styles.count}>{attendance.length} employees</span>
            <button style={styles.exportBtn} onClick={() => exportToExcel(attendance, "Attendance_Report")}>
              📥 Export Excel
            </button>
          </div>
          <div style={styles.tableWrapper}>
            <table style={styles.table}>
              ...
              <tbody>
                {attendance.length === 0 ? (
                  <tr><td colSpan={9} style={styles.empty}>No attendance data for this month</td></tr>
                ) : attendance.map((a, i) => (
                  <tr key={i} style={styles.tr}>
                    <td style={styles.td}>{i + 1}</td>
                    <td style={styles.td}><span style={styles.empName}>{a.name}</span></td>
                    <td style={styles.td}><span style={styles.deptBadge}>{a.department_name || "—"}</span></td>
                    <td style={styles.td}><span style={{ color: "#059669", fontWeight: "700" }}>{a.present}</span></td>
                    <td style={styles.td}><span style={{ color: "#dc2626", fontWeight: "700" }}>{a.absent}</span></td>
                    <td style={styles.td}><span style={{ color: "#d97706", fontWeight: "700" }}>{a.half_day}</span></td>
                    <td style={styles.td}><span style={{ color: "#1d4ed8", fontWeight: "700" }}>{a.late}</span></td>
                    <td style={styles.td}>{a.total_hours || 0}h</td>
                    <td style={styles.td}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        <div style={{ flex: 1, height: 8, backgroundColor: "#f1f5f9", borderRadius: 4, overflow: "hidden", minWidth: 60 }}>
                          <div style={{ height: "100%", width: `${a.weightage || 0}%`, backgroundColor: getWeightageColor(a.weightage), borderRadius: 4 }} />
                        </div>
                        <span style={{ fontSize: 13, fontWeight: "700", color: getWeightageColor(a.weightage) }}>
                          {a.weightage || 0}%
                        </span>
                      </div>
                    </td>
      ...
      {tab === "charts" && !loading && (
        <div style={styles.chartsGrid}>
            ...
            {deptStats.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height={260}>
                <BarChart data={deptStats} margin={{ bottom: 40 }}>
                  ...
                  <XAxis dataKey="department_name" tick={{ fontSize: 11 }} angle={-20} textAnchor="end" />
                  <YAxis tick={{ fontSize: 11 }} />
                  <Tooltip />
                  <Bar dataKey="total_employees" name="Employees" radius={[4,4,0,0]}>
            ...
            {deptStats.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height={260}>
                <BarChart data={deptStats} margin={{ bottom: 40 }}>
                  ...
                  <Tooltip formatter={(val) => [`₹${Number(val).toLocaleString()}`, "Avg Salary"]} />
                  <Bar dataKey="avg_salary" name="Avg Salary" fill="#1d4ed8" radius={[4,4,0,0]} />
                </BarChart>
              </ResponsiveContainer>
            )}
          </div>
          ...
          <div style={styles.chartCard}>
            <h3 style={styles.chartTitle}>📊 Total Salary Cost by Department</h3>
            {deptStats.length === 0 ? (
              <div style={styles.noData}>No data available</div>
            ) : (
              <ResponsiveContainer width="100%" height={260}>
                <BarChart data={deptStats} margin={{ bottom: 40 }}>
                  ...
                  <XAxis dataKey="department_name" tick={{ fontSize: 11 }} angle={-20} textAnchor="end" />
                  <YAxis tickFormatter={v => `₹${(v/1000).toFixed(0)}k`} tick={{ fontSize: 11 }} />
                  <Tooltip formatter={(val) => [`₹${Number(val).toLocaleString()}`, "Total Salary"]} />
                  <Bar dataKey="total_salary" name="Total Salary" fill="#059669" radius={[4,4,0,0]} />
                </BarChart>
              </ResponsiveContainer>
            )}
          </div>
...
const styles = {
  ...
  tab: { padding: "8px 20px", backgroundColor: "#e2e8f0", border: "none", borderRadius: 8, cursor: "pointer", fontSize: 14, fontWeight: "600" },
  activeTab: { backgroundColor: "#1d4ed8", color: "#fff" },
  errorBox: { backgroundColor: "#fee2e2", color: "#dc2626", padding: 16, borderRadius: 8, marginBottom: 16, fontWeight: "600" },
  loadingBox: { backgroundColor: "#eff6ff", color: "#1d4ed8", padding: 16, borderRadius: 8, marginBottom: 16, textAlign: "center", fontWeight: "600" },
  exportBar: { display: "flex", alignItems: "center", gap: 12, marginBottom: 14, flexWrap: "wrap" },
  count: { fontSize: 13, color: "#64748b", fontWeight: "500" },
  filterSelect: { padding: "8px 12px", borderRadius: 6, border: "1.5px solid #bfdbfe", fontSize: 14, backgroundColor: "#fff" },
  exportBtn: { padding: "8px 18px", backgroundColor: "#059669", color: "#fff", border: "none", borderRadius: 8, cursor: "pointer", fontWeight: "600", marginLeft: "auto" },
  ...
  chartsGrid: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(420px,1fr))", gap: 20 },
  chartCard: { backgroundColor: "#fff", borderRadius: 12, padding: 24, boxShadow: "0 2px 8px rgba(29,78,216,0.06)", border: "1px solid #dbeafe" },
  chartTitle: { fontSize: 15, fontWeight: "700", color: "#1e3a8a", marginBottom: 16 },
