...
function ManageRoles() {
  ...
  const fetchUsers = async () => {
    try {
      const res = await axios.get("http://localhost:5000/api/user/all", { headers: { Authorization: token } });
      setUsers(res.data);
    } catch (err) { handleApiError(err, navigate); }
  };

  useEffect(() => { fetchUsers(); }, [token, navigate]);

  const handleRoleChange = async (id, newRole) => {
    try {
      await axios.put(`http://localhost:5000/api/user/role/${id}`, { role: newRole }, { headers: { Authorization: token } });
  ...
  const getRoleBadgeStyle = (role) => {
    if (role === "admin")   return { backgroundColor: "#fee2e2", color: "#dc2626" };
    if (role === "manager") return { backgroundColor: "#ffedd5", color: "#d97706" };
          ...
          <tbody>
            {users.map((u, i) => (
              <tr key={u.id} style={styles.tr}>
                ...
                <td style={styles.td}><span style={{ fontWeight: "600", color: "#1e3a8a" }}>{u.name}</span></td>
                ...
                <td style={styles.td}>
                  <select style={styles.select} value={u.role || "employee"} onChange={(e) => handleRoleChange(u.id, e.target.value)}>
...
const styles = {
  ...
  select: { padding: "7px 12px", borderRadius: 6, border: "1.5px solid #bfdbfe", fontSize: 14, cursor: "pointer", backgroundColor: "#f8fafc" },
};
